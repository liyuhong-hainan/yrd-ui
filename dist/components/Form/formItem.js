...
var FormItem = function (props) {
    ...
    var rowClass = classNames('yrd-row', {
        'yrd-row-no-label': !label
    });
    useEffect(function () {
        var value = (initialValues && initialValues[name]) || '';
        dispatch({ type: 'addField', name: name, value: { label: label, name: name, value: value, rules: rules || [], errors: [] } });
    ...
    var labelClass = classNames({
        'yrd-form-item-required': isRequired
    });
    var itemClass = classNames('yrd-form-item-control', {
        'yrd-form-item-has-error': hasError
    });
    // 1.手动创建一个属性列表，需要有 value 以及 onChange 属性
    ...
    return (React.createElement("div", { className: rowClass },
        label &&
            React.createElement("div", { className: "yrd-form-item-label" },
                React.createElement("label", { title: label, className: labelClass }, label)),
        React.createElement("div", { className: "yrd-form-item" },
            React.createElement("div", { className: itemClass }, returnChildNode),
            hasError &&
                React.createElement("div", { className: 'yrd-form-item-explain' },
