...
const FormItem: React.FC<FormItemProps> = (props) => {
    ...
    const rowClass = classNames('yrd-row', {
        'yrd-row-no-label': !label
    })
    useEffect(() => {
        const value = (initialValues && initialValues[name]) || ''
        dispatch({ type: 'addField', name, value: { label, name, value: value, rules: rules || [], errors: [] }})
    ...
    const itemClass = classNames('yrd-form-item-control', {
    ...
    return(
        <div className={rowClass}>
            { label && 
                <div className="yrd-form-item-label">
                    <label title={label} className={labelClass}>
                        {label}
                    </label>
                </div>
            }
            <div className="yrd-form-item">
                <div className={itemClass}>
                    {returnChildNode}
                </div>
                {
                    hasError &&
                    <div className='yrd-form-item-explain'>
